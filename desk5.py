...
class DocumentProcessorApp(QMainWindow):
    ...
    def upload_documents(self):
        ...
        if filenames:
            ...
            for filename in filenames:
                ...
                c.execute("INSERT INTO documents (filename, file_content, text_content) VALUES (?, ?, ?)", (destination, file_content, text_content))
                conn.commit()
    ...
    def retrieve_data(self):
        ...
        c.execute("SELECT id, text_content, filename FROM documents")
        all_documents = c.fetchall()

        similarities = compare_text_with_jd(self.provided_text, {doc[0]: doc[1] for doc in all_documents})
        print(similarities)

        self.ranked_documents = [(all_documents[similarity[0]][2], similarity[1]) for similarity in similarities]
